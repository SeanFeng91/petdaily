...
export function getBarkFrameDecision(features = {}, sensitivity = "low", noiseBaseline = 0.025) {
  ...
  const strongBarkOverride =
    features.relativeRms >= config.relativeRms * 1.42 ||
    features.spectralFlux >= config.flux * 1.85 ||
    features.peak >= Math.max(noiseBaseline * config.peakMultiplier * 1.45, features.rms * 2.8);
  const accepted = score >= config.minScore && loudEnough && attack && (!speechLike || strongBarkOverride);
