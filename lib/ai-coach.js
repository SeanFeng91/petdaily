...
import { createInsight, getCoachContext } from "@/lib/pet-store";
...
export async function generateCoachInsight(petId) {
  const { pet, timelineEvents, weightRecords, reminders, expenses } = await getCoachContext(petId);
...
async function persistInsight(petId, insight) {
  return createInsight(petId, insight);
}
