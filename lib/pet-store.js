...
function normalizeBarkSample(row) {
  return row
    ? {
        ...
        features: normalizeStructuredField(row.features),
...
async function updateBarkSessionTimeline(runtime, { session, sample, profile }) {
  ...
  if (runtime.kind === "d1") {
    ...
    return d1First(runtime.db, 'SELECT * FROM "TimelineEvent" WHERE "id" = ?', [session.timelineEventId]);
  }

  return runtime.db.timelineEvent.update({
...
export async function createBarkSample(body, audioFile) {
  ...
  if (runtime.kind === "d1") {
    await d1Run(
      runtime.db,
      'INSERT INTO "BarkSample" ("id", "petId", "clusterId", "sessionId", "timelineEventId", "audioUrl", "audioObjectKey", "audioContentType", "audioSizeBytes", "features", "embedding", "waveform", "barkScore", "detectorVersion", "status", "reason", "note", "capturedAt", "durationMs", "createdAt") VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [
        sample.id,
        sample.petId,
        sample.clusterId,
